...
function mergeObjects(objects) {
  const allEntries = objects.flatMap((item) => Object.entries(item));
  return allEntries.reduce((acc, [key, value]) => {
...
function compareObjects(obj1, obj2) {
  ...
  return (
    keys1.every((value, i) => value === keys2[i]) &&
    values1.every((value, i) => value === values2[i])
  );
}
...
function makeWord(lettersObject) {
  ...
  const lettersArr = entries.reduce((acc, item) => {
    for (let i = 0; i < item[1].length; i += 1) {
      acc.push([item[0], item[1][i]]);
    }
    return acc;
  }, []);
  lettersArr.sort((a, b) => a[1] - b[1]);
  const str = lettersArr.map((item) => item[0]).join('');
  return str;
}
...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    ...
    if (bill === 25) wallet25 += 1;

    if (bill === 50) {
      if (wallet25 === 0) ok = false;
      else {
        wallet25 -= 1;
        wallet50 += 1;
      }
